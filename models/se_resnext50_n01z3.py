...
def get_net(args):
    ...
    if args.num_input_channels != 3:
        ...
        for i in range(int(args.num_input_channels / 3)):
            model.conv1.weight.data[:, i * 3: (i + 1) * 3] = conv1_.weight.data / (int(args.num_input_channels / 3))

        if args.num_input_channels % 3 > 0: 
            model.conv1.weight.data[:, - (args.num_input_channels % 3) : ] = conv1_.weight.data[:, -(args.num_input_channels % 3) :]
